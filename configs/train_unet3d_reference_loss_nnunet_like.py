...
# model
cfg.model_name = "Unet3D_ref_loss_diceBCE_nnunet_like"
cfg.spatial_dims = 3
cfg.in_channels = 1
cfg.out_channels = 3
...
def build_train_transforms(*, cfg, case_day_slices, rle_index):
    return Compose(
        [
            ...
            # nnU-Net-like spatial augmentation: moderate rotate/scale, no elastic deformation.
            RandAffined(
                keys=["image", "label"],
                prob=0.35,
                rotate_range=(float(np.pi / 6.0), float(np.pi / 6.0), float(np.pi / 6.0)),
            ...
            # Keep depth flip disabled for this task even though nnU-Net often allows all axes.
            ...
            RandShiftIntensityd(keys=["image"], offsets=(-0.1, 0.1), prob=0.15),
